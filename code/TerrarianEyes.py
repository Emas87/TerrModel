...
class TerrarianEyes:
    ...
    def findTiles(self, source):
        ...
        for _,row in df.iterrows():
            if row['name'] == "dirt":
                images = self.grassImages
            elif row['name'] == "stone":
                images = self.stoneImages
            ...
            for rectangle in rectangles:
                x1 = int(round(row.xmin) + rectangle[0])
                #x2 = int(round(row.xmin) + rectangle[0] + rectangle[2])
                y1 = int(round(row.ymin) + rectangle[1])
    ...
    def loadModels(self):
        self.tiles_model = torch.hub.load('.', 'custom', path=tiles_weights_path, source='local')
        self.objects_model = torch.hub.load('.', 'custom', path=objects_weights_path, source='local')
    
    def detectYoloTiles(self, source):
        # Load model
        device = select_device('')
        dnn = False
        data = '../datasets_objects/data.yaml'
        half = False
        imgsz = [640, 640]
        conf_thres = 0.25
        iou_thres = 0.45
        ...
        model = DetectMultiBackend([self.tiles_weights_path], device=device, dnn=dnn, data=data, fp16=half)
    ...
    def loadImages(self):
        ...
        for file in files:
            # iterate one by one
            self.grassImages.append(os.path.join('.',dir,file))
        dir = "stone"
        files = os.listdir(dir)
        for file in files:
            # iterate one by one
            self.stoneImages.append(os.path.join('.',dir,file))
    ...
    def startController(self, window_name):
        ...
        while(True):
            ...
            # do object detection
            #rectangles = self.matchImage(screenshot, images)
            rectangles = self.findTiles(screenshot)
